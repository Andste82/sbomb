...
- Sections 19, 20 and 22: files are mapped to components by the priority chain
  ...
  a licence file, the anchor root, then an explicit unknown component);
  ...
  `COMPONENT_ROOT_UNRESOLVED`. Strategies 4 (SDK layout) and 5 in its automatic
  form remain open, and `CMakeLists.txt` is deliberately not a marker (D27,
