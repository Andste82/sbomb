...
* A CI matrix job running the same fixture on `ubuntu-latest` (x86_64) and `ubuntu-24.04-arm` (ARM64) and comparing output hashes. **There is no Windows runner**; the Windows binary is cross-compiled, smoke-tested only by `sbomb version` under Wine if available, and otherwise declared untested in `docs/dev/status.md`.
...
**Definition of Done:** byte-identical output across architectures for the portable fixture, and Windows path semantics proven by flavor tests rather than by a Windows runner. Any Windows-specific behaviour that cannot be expressed as a flavor test MUST be listed in `docs/dev/status.md` as unverified.
