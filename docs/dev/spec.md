...
2. Exact package-manager metadata (Conan, vcpkg, CPM, FetchContent, ESP-IDF component manager, west, Meson).
...
6. Nearest ancestor directory containing recognized package metadata (`conanfile.py/txt`, `vcpkg.json`, `CMakeLists.txt` with `project()`, `package.json`-like SDK manifests, `idf_component.yml`, `Cargo.toml` for mixed repos).
   - **6a.** The pkg-config metadata a system library installed beside itself, for a file under a `sysroot:*` or `toolchain:*` anchor. It is numbered `6a` rather than `7` so that every reference to strategies 7 and 8 elsewhere in this document keeps its meaning.
7. Anchor root itself (e.g. everything under `pkg:conan/mbedtls` maps to that package).
8. Unknown component.

Strategy 6 recognizes a directory as a component root by the files it carries: `conanfile.py/txt`, `vcpkg.json`, `idf_component.yml`, `Cargo.toml`, `west.yml`, `CONTROL` — **a recognized licence file that grants a licence**, matched as §22.3 matches one: case-insensitively, with the optional `.txt`/`.md` extension, and including the `LICENSE-<id>` form — **and a bundled SBOM under a conventional name** (`sbom.cdx.json`, `bom.cdx.json`, `sbom.spdx.json`, `bom.spdx.json`). A directory that carries its own licence is a distinct work by convention, and for a library that was simply copied into the source tree it is the only marker there is; a dependency that ships its own SBOM says the same thing, and some ship nothing else. Neither marks a boundary at the anchor root itself: a project's own top-level licence, and a project's own SBOM, describe the project rather than a dependency inside it.

The matching rule is §22.3's and not a list of names, because a component that carries `LICENSE-MIT` and `LICENSE-APACHE` side by side carries no file called `LICENSE` at all. A boundary decided by exact names left such a library invisible: its sources dissolved into the enclosing project and the attribution export named the manufacturer as the holder of both texts. Where a directory carries several, the boundary is named after the first in the consultation order of §22.3, so that one run of one evidence set names one marker.
...
The licence marker is the one exception, because its matching rule cannot be expressed as a name to `stat`, and it stays within the same budget by a different route: the walk asks about the same ancestor directory once per file under it, so the answer per directory MUST be memoized for the run. One listing per directory is then fewer reads than the `stat` per candidate name it replaces.

`NOTICE` and `COPYRIGHT` are not boundary markers, although §22.3 recognizes both as licence files. They are attribution material rather than a licence grant, and a directory carrying only a NOTICE is not thereby a separate work. The three that do mark a boundary are `LICENSE`, `LICENCE` and `COPYING`, plus `LICENSE-<id>`.

Neither are the files the root-manifest readers of §21 look for -- `<Pkg>ConfigVersion.cmake`, a build2 `manifest`, `xmake.lua`, `MODULE.bazel`. They are read **in** a root that is already settled and never draw one: widening this list moves component boundaries and costs a `stat` per ancestor directory per used file against §31, so a library carrying only an `xmake.lua` gets no component of its own (D40).
...
Strategy 2 works on two kinds of statement, and the more precise one decides first. A manager that records **which files it installed** (§21) names them outright, and that lookup outranks any prefix: inside a vcpkg triplet tree every port's headers land in the same `include/` directory, where no root can tell them apart. Only then do the package **roots** decide. A package MAY have several roots — FetchContent populates `_deps/<name>-src` with the checkout and `_deps/<name>-build` with what CMake generated for it, and both are the package — but exactly one of them is the *identity* root: the one the anchor of §7.2 names and the one the component root, the licence file and any VCS question refer to. A file two packages both claim belongs to neither; two statements are no statement, and choosing between them would be a guess. The file then falls through to the strategies below, which may put it in a third component, so dropping it MUST emit `COMPONENT_MAPPING_CONFLICT` (info) naming both packages and the manager each came from.

Strategy 5 works the same way and reports the same identifier: a source two targets both list belongs to neither of them, because the build system said two things and picking one would be a guess. The finding names every target that listed the source.

**Strategy 6a is the one place a system file can be named after its package.** Without it every file under one sysroot falls into a single component named after the anchor (strategy 7), for which no version could be stated at all. It applies only to a file whose anchor is a `sysroot:*` or a `toolchain:*`, and only where the strategies above found nothing. From that file it walks **up**, bounded by its anchor, and at each level looks in `<dir>/pkgconfig` and `<dir>/share/pkgconfig` for a .pc file addressed **by name**: the name derived from the used file itself — `foo.pc` and `libfoo.pc` for `libfoo.so.3`, the containing directory's and the header's own stem for a header. No directory is listed and no set of .pc files is searched; a package whose module name cannot be derived from the file's name is not found, and that silence is the intended answer (D37).

A candidate is used only after it **verifies** against the file: for a library, a directory it names as a library directory has to be the directory the file really lies in, *and* one of its `-l` names has to name that file; for a header, an include directory it names has to contain the file. Absolute directories a .pc file states are resolved inside the anchor first, the way `PKG_CONFIG_SYSROOT_DIR` does, and a directory with a `..` segment or one outside the anchor is refused (§30.3). Where two candidates verify and describe the file differently, **nothing is mapped** — two statements are no statement — the file keeps the component strategy 7 gives it, and the disagreement is reported as `COMPONENT_MAPPING_CONFLICT` (info) naming both files. A file that verifies against nothing is silence.

The same rule holds once more over the whole run, because one module name can be mapped from more than one file. Two packages of one name in one sysroot — a distribution `libfoo` under `/usr/lib` and a hand-built one under `/opt/lib`, each verified against the .pc file beside itself — name one component, and each answer is right for its own file. Where they state the same thing that is one answer given twice. Where they state different things the mapping stands, because both files really do belong to a package of that name, but the component MUST carry the value of neither and the disagreement MUST be reported as `COMPONENT_MAPPING_CONFLICT` (info) against the component, naming each distinct answer and a file that gave it. Publishing one of the two would be a value that is wrong for the other file and said nothing about.

The mapped component keeps the **scope of its anchor**, `system` or `toolchain`: §24.1 keeps such a file out of the product's dependencies, and naming it does not make it one. It has **no component root** at all — a .pc file names where the package installed its libraries, not a directory the component owns — and that is a resolved answer rather than a gap, so `COMPONENT_ROOT_UNRESOLVED` does not fire for it and no licence is looked for in the sysroot.

**The component root is a resolved fact, not a recomputation.** Every mapped component has a root, settled by the same strategy that named it: the curated `components[].path`, the package-manager root, the marker directory strategy 6 stopped at, or the anchor root when the anchor *is* the component. The deepest common directory of the component's *used* files is the last resort, and using it emits `COMPONENT_ROOT_UNRESOLVED` (info) — because that directory moves with whichever files the linker kept, so a licence or a version read from it would move too. The resolved root is published as `sbomb:component:root`. The one exception is a component strategy 6a named: it has **no** root, because the .pc file that named it names no directory the component owns, and a component with no root carries no `sbomb:component:root` and emits no finding about it.
...
|---|---|
| `curated` | `attestation` |
| `cmake`, `conan`, `vcpkg`, `fetchcontent`, `bundled-sbom`, `idf`, `cpm`, `yocto`, `buildroot`, `pkg-config`, `cmsis-pack`, `west`, `cmake-config-version`, `build2`, `xmake`, `bazel`, `meson` | `manifest-analysis` |
...
| Meson `[wrap-git]` | `pkg:generic/<name>@<version>?vcs_url=git%2B<url>%40<commit>` |
| Meson `[wrap-file]` | no purl; emit `UNKNOWN_PURL` (informational) |
| A root manifest (`cmake-config-version`, `build2`, `xmake`, `bazel`) | no purl; emit `UNKNOWN_PURL` (informational) |
...
The **Meson** adapter reads the wrap files a project declares for its subprojects: `<source>/subprojects/*.wrap`, and no other location. A wrap's own file name is the subproject name Meson addresses it by and is therefore the component's name; `directory` names the tree the subproject was unpacked into, defaults to that same name, is resolved inside `subprojects/` and is refused when it leaves it (§30.3). A `[wrap-git]` states `revision`, which is read exactly as a west project's is: a commit travels in the purl and in `vcsCommit` and is never published as a version, while anything else is a declared version at rank 2 of §21.1, and `url` is the repository. A `[wrap-file]` states a tarball URL and a file name; it contributes **no** version, because reading one out of an archive's file name would be a guess, and **no** purl, because a downloaded archive names no package ecosystem -- such a subproject keeps `UNKNOWN_VERSION` and `UNKNOWN_PURL`. Only the first `wrap-*` section is read: a file declaring both kinds contradicts itself about how the subproject was obtained, and the keys of the two mean different things. `[provide]`, `patch_directory` and `diff_files` are not read at all -- they say how Meson patches and how it maps a dependency name, not what the dependency is. A wrap whose directory is **not on disk** is silence rather than missing evidence: Meson unpacks a subproject while it configures, so such a wrap describes a dependency this build never obtained. The file is subject to the bounds of §30, in bytes per wrap and in the number of wraps, and a project that declares more than the limit is refused whole. A Meson subproject is very often a git submodule as well; the fixed adapter order asks Meson first, because a wrap names the subproject, the revision and the repository where `.gitmodules` states a path and a URL (D40).
...
The next such readers are the four **root manifests**: the single file a build system keeps at the root of the thing it builds. They are `<Pkg>ConfigVersion.cmake` or `<pkg>-config-version.cmake` (the `PACKAGE_VERSION` a CMake package-version file assigns), a build2 `manifest`, an `xmake.lua` (`set_version`) and a `MODULE.bazel` (the `version` of its `module()` call). Each contributes `version` at rank 2 of §21.1, and build2 also contributes `license`; none of them contributes a `supplier` or a `purl`, so `MISSING_SUPPLIER` and `UNKNOWN_PURL` remain for such a component.

**No language is interpreted.** CMake, Lua and Starlark are programming languages, and evaluating one to learn a version would mean running somebody else's code. Each reader therefore recognises exactly one assignment in exactly one shape and knows nothing else about the file: a value that is a variable reference, a file that states the assignment twice with different values, a `module()` call that is not closed, and a root holding two package-version files are all `EVIDENCE_UNREADABLE` and contribute nothing, because choosing between two statements or expanding one is the guess §20.1 forbids. Each file is bounded separately in bytes, in lines and in line length (§30); a breach is `INPUT_LIMIT_EXCEEDED` and the file is not read. A declaration inside a **comment** is not a declaration, and comments are lexed rather than matched line by line: Lua's long comment (`--[[ ... ]]`, with any number of `=` between the brackets) spans lines that carry no marker of their own, and a `set_version` inside one is a release somebody commented out, not the version of the component. For the same reason a parenthesis inside a string literal does not close a `module()` call.

**A name is never published**, from any of them: `Field` has no entry for it and a component keeps the name settled before its files were grouped (D33). A build2 `summary` has nowhere to go at all. A build2 `license` is published only where the value has the **shape** of an SPDX expression -- identifiers alternating with `AND`, `OR`, `WITH`, parentheses balanced -- because `licenses[].expression` is defined as one and build2's `other: <description>` form and any free text are not; a value of another shape is dropped in silence, exactly as a CMSIS `<license>` path is. Because the file name `manifest` belongs to half the world -- a Yocto image manifest, a container manifest, a text file somebody called manifest -- a build2 manifest is recognised by the format line `: 1` it must open with, and a file of that name without it is passed over in **silence** rather than claimed. A root with none of these files is silence too, and so is one whose file states no version. Rank 2 is a deliberate understatement for the generated CMake file, which §21.1 would otherwise call install state: a claim ranked too low can only lose to a better origin, while one ranked too high would overrule the manager that really installed the package (D40). None of these names is a boundary marker of §19.2, and the order of the readers among themselves is behaviour: they rank alike, so a root holding two of them publishes the version of whichever reader the registry asks first.
...
And there is a reader of a **fourth** kind, distinguished again by what it is handed: not a manager's install layout, not a settled root and not a file the user named, but a used **file** and the anchor that bounds it. The first of these reads the **pkg-config metadata** a system library installs beside itself — `<libdir>/pkgconfig/<module>.pc`, a few `name=value` variable definitions and `Key: value` lines, `${var}` references and `#` comments. Nothing is configured and nothing is searched for: the reader walks up from the used file, bounded by its anchor, and addresses a .pc file by a name derived from that file, as §19.2 strategy 6a describes. It contributes `version` at rank 3 of §21.1 — a .pc file is the generated configuration a package writes while installing — and `name` only in the sense that the module it was addressed by becomes the component's name through the mapping; the `Name:` line itself is never published, because it is a display name and the module is the identity (`xkeyboard-config.pc` states `Name: XKeyboardConfig`). It contributes **no** licence, supplier or purl, because a .pc file states none, so `UNKNOWN_LICENSE`, `MISSING_SUPPLIER` and `UNKNOWN_PURL` remain for such a component. Like an enricher it returns a name and claims and no paths, so it can neither add a file to the used set, nor register an anchor, nor move a boundary. It is subject to the bounds of §30 and is applied whole or not at all: a file over a ceiling, or one whose variable expansion cannot terminate, is reported as `INPUT_LIMIT_EXCEEDED`; one whose structure this tool cannot read, or which states neither a `Name` nor a `Version`, as `EVIDENCE_UNREADABLE`. A file that is simply not there is silence, and so is one whose statements do not verify against the used file. Each .pc file is read once per run and reported once (§31).
...
The **pkg-config** reader of §21 closes part of this gap from files alone, with no permission and no process: where a distribution installed a `.pc` file that verifies against a used library or header, §19.2 strategy 6a names that file's component after the pkg-config module and gives it the version the file states, instead of folding it into the one component named after the sysroot. It closes only part: a .pc file names no distribution, no package format and no architecture, so no purl is asserted, and it names no licence, so the licence of such a component stays open. Introspection remains the only way to the rest.
