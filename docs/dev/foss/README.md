...
> **This plan was carried out.** All eight milestones are implemented,
> reviewed against it and merged (issue #24, pull request #39). What follows is
> the plan as it was written, kept because it records *why* — the requirements
> with their sources, the gap analysis against the code as it then was, and the
> decisions. What was actually built is in `docs/CHANGELOG.md`, the
> specification, `docs/dev/deviations.md` and `docs/dev/open-questions.md`;
> where the two disagree, those are current and this is history.
