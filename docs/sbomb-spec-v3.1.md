...
### 41.0 Universal Milestone Gate

Every milestone MUST satisfy all of the following before it is considered done. These are not repeated per milestone.
...
Additionally, every milestone MUST:

* leave the repository in a shippable state (`cmd/sbomb` builds and `sbomb version` works);
* add at least one golden or table-driven test per new behaviour;
* not implement behaviour belonging to a later milestone;
* update `docs/CHANGELOG.md` and `docs/status.md` (which milestone is complete, what is stubbed);
* record any unspecified decision in `docs/open-questions.md` rather than guessing.

Milestones 0–16 are the core track and MUST be implemented in order. Milestones 17, 19, and 20 are adapter tracks and MAY be implemented in any order after Milestone 16.

**Milestones 18 (MSVC/MSBuild) and 21 (IAR and vendor linkers) are PARKED.** An implementing agent MUST NOT start them. They remain in this document so the design constraints they impose on earlier milestones stay visible.
