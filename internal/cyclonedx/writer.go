...
func componentToCyclone(component domain.Component, ref, specVersion string, options sbomwriter.Options) Component {
	...
	for _, exclusion := range component.CVEExclusions {
		out.Properties = append(out.Properties, Property{Name: "sbomb:component:cve-exclusion", Value: exclusion.CVE + ": " + exclusion.Reason})
	}
...
func techniqueForVersionSource(source string) string {
	...
	case "conan", "vcpkg", "fetchcontent", "cmake", "bundled-sbom", "idf", "cpm", "yocto", "buildroot", "pkg-config", "cmsis-pack", "west",
		...
		// as well, however much the build knew when it wrote them. pkg-config
		// is the .pc file a system library installs beside itself: the
		// generated configuration a package writes while installing, which is
